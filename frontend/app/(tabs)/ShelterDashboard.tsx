...
const REPORT_LABELS: Record<string,string> = { access:"Access", capacity:"Capacity", cleanliness:"Cleanliness", pet_issue:"Pets", damage:"Damage", other:"General" };
...
const CHIPS  = ["Recently Reported", "Full", "Pet Friendly 🐾", "Accessible ♿", "All"];
...
const dr = StyleSheet.create({
  ...
  menuTitle:{ color:"#666", fontSize:12, textAlign:"left", paddingHorizontal:20, paddingTop:14, paddingBottom:8 },
...
export default function ShelterDashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    if (chip === "Accessible ♿")       list = list.filter(s => s.isAccessible && !s.hasStairs);
    if (chip === "Pet Friendly 🐾")     list = list.filter(s => !s.petIssueReported);
    if (chip === "Full")                list = list.filter(s => s.isFull);
    if (chip === "Recently Reported")   list = list.filter(s => !!s.lastReportAt);
    return list;
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.statsRow}>
        {[
          { label:"Closed / Locked", value:closedCount,     color:"#E24B4A", main:false },
          { label:"Busy / Full",     value:fullCount,       color:"#BA7517", main:false },
          { label:"Available",       value:openCount,       color:"#1D9E75", main:false },
          { label:"Total Shelters",  value:filtered.length, color:"#fff",    main:true  },
        ].map(c => (
...
<View style={[s.cellV, {width:COLS[1].width}]}>
  ...
  <Text style={s.cell} numberOfLines={1}>{item.neighborhood || "—"}</Text>
                   ...
                   {/* Name */}
                    ...
                    <Text style={[s.bold, {color:"#fff", textAlign:"left", paddingHorizontal:8}]} numberOfLines={1}>
    {item.name}
  </Text>
  <View style={{flexDirection:"row", gap:4, paddingHorizontal:8, marginTop:2, justifyContent:"flex-start"}}>
      ...
      <Modal transparent visible={showAdd} animationType="slide" onRequestClose={() => setShowAdd(false)}>
        <Pressable style={s.modalOverlay} onPress={() => setShowAdd(false)}>
          <Pressable style={s.modalCard} onPress={(e)=>e.stopPropagation()}>
            ...
            <TextInput style={s.modalInput} placeholder="Shelter name" placeholderTextColor="#666"
              value={newShelter.name} onChangeText={t => setNewShelter({...newShelter, name:t})} />
            <TextInput style={s.modalInput} placeholder="Address" placeholderTextColor="#666"
              value={newShelter.address} onChangeText={t => setNewShelter({...newShelter, address:t})} />
            <TextInput style={s.modalInput} placeholder="Neighborhood" placeholderTextColor="#666"
              value={newShelter.neighborhood} onChangeText={t => setNewShelter({...newShelter, neighborhood:t})} />
            <TextInput style={s.modalInput} placeholder="Area" placeholderTextColor="#666"
              value={newShelter.area} onChangeText={t => setNewShelter({...newShelter, area:t})} />
            <TextInput style={s.modalInput} placeholder="Capacity" placeholderTextColor="#666"
...
const s = StyleSheet.create({
 ...
 chipsRow: { flexDirection:"row", gap:10, marginBottom:14, flexWrap:"wrap", justifyContent:"flex-start" },
  ...
  chipTxtOn:   { color:"#fff" },
  headerRow:   { flexDirection:"row", backgroundColor:"#242424", borderTopLeftRadius:12, borderTopRightRadius:12, borderWidth:0.5, borderColor:"#333", paddingVertical:12, justifyContent:"flex-start" },
 ...
 cellV: { paddingHorizontal:8, justifyContent:"center", alignItems:"center" },
  ...
  modalTitle:  { color:"#fff", fontSize:18, fontWeight:"600", marginBottom:14, textAlign:"left" },
  modalInput:  { backgroundColor:"#1c1c1c", color:"#fff", borderRadius:8, paddingHorizontal:12, paddingVertical:10, marginBottom:10, fontSize:15, textAlign:"left", borderWidth:0.5, borderColor:"#333" },
